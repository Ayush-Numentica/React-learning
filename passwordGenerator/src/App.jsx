...
function App() {
  ...
  const passwordGenerator = useCallback(() => {
    let pass = "";
    let str = "ABCDEFGHIJKLMNOPQRSTUVWXIZabcdefghijklmnopqrstuvwxyz";
  ...
  const copyPasswordToClipboard=useCallback(()=>{
    ...
    // passwordRef.current?.setSelectionRange(0,3);//select only three values that is to copy only three starting data
    window.navigator.clipboard.writeText(password);
  },[password])

  useEffect(() => {
    passwordGenerator()
  }, [length, numberAllowed, charfacterAllowed, passwordGenerator])
